...
class LLMAgent:
    ...
    def chat(self, user_input: str) -> str:
        ...
        try:
            ...
            if any(word in user_input.lower() for word in ['select', 'sql', 'query', 'table', 'schema', 'database', 'employees', 'departments', 'orders']):
                # Try RAG search first
                if self.query_engine:
                    try:
                        rag_response = self.query_engine.query(user_input)
                        response_text = str(rag_response)
                        logger.info("Used RAG query engine for response")
                    except Exception as e:
                        logger.warning(f"RAG search failed: {e}")
            
            # If no RAG response or general query, use direct LLM
            if not response_text:
                # Build context from memory
                context = memory.get_context_string()
                
                # Create prompt
                prompt = f"""You are a helpful database assistant. 
